...
- 判断DB: `/Users/kurosawa/Dev/ai-service/catalog/services/github-copilot.md`
...
GitHub Copilot では、`copilot-instructions.md` が Rule の中核です。
最初に Rule を固定しないと、補完とレビューの品質がぶれます。

### Rule の優先順位

| レイヤー | 例 | 使い方 |
| --- | --- | --- |
| セッション指示 | Copilot Chat の依頼文 | その作業だけの一時指示 |
| リポジトリ指示 | `.github/copilot-instructions.md` | プロジェクト共通ルール |
| 組織ポリシー | Copilot Business/Enterprise 設定 | 組織全体の強制ルール |
...
- セキュリティ制約: 機密情報を提案・ログ・コメントに含めない

### Rule 追記例

```markdown
## セキュリティ
- ハードコードされたトークンや秘密情報を生成しない
- SQLは必ずパラメータ化クエリを使う

## 品質
- 変更後はテストコマンドを明示する
- 既存 API の互換性を壊す変更は PR に明記する
```

## 6-2. Skills（再利用できる手順の基本機能）

Copilot の Skills は、レビュー・テスト生成・説明などの定型作業を再利用するための仕組みです。
...
Copilot Edits/Chat を使う場合も、役割を分けて依頼すると品質が安定します。
...
| レビュー Agent | PR 差分の監査、回帰リスク抽出 | 実装修正の直接実行 |
| ドキュメント Agent | PR 要約、変更手順、運用手順更新 | コードの仕様変更 |

### Agent 指示の書き方

- 目的: 何を達成するか
- 入力: 参照ファイル、差分、ログ
- 完了条件: テスト結果、報告形式、未解決課題
